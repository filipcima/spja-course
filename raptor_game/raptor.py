...
class Raptor(pygame.sprite.Sprite):
    ...
    images = {
        'left': 'images/plane_turning_right_1.gif',
    ...
    def load_images(self):
        ...
        self.loaded_images['left'] = load_image(self.images['left'], -1)
        self.loaded_images['straight'] = load_image(self.images['straight'], -1)
        self.loaded_images['right'] = load_image(self.images['right'], -1)
    ...
    def update(self, event):
        ...
        if event.type == KEYDOWN:
            if event.key == K_RIGHT:
                ...
                self.image, self.rect = self.loaded_images['right']
                self.rect.center = center
            elif event.key == K_LEFT:
                move_x = -self.x_dist
                center = self.rect.center
                self.image, self.rect = self.loaded_images['left']
                self.rect.center = center
            elif event.key == K_UP:
                move_y = -self.y_dist
            ...
            center = self.rect.center
            self.image, self.rect = self.loaded_images['straight']
            self.rect.center = center
